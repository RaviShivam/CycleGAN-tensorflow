...
def load_pretrained_model(MODEL_NAME='mobilenetv2_coco_voctrainaug'):

    ROOT_DIR = os.path.dirname(os.path.abspath(__file__))
    MODEL_DIR = os.path.join(ROOT_DIR, 'models')
    _TARBALL_NAME = '{}.tar.gz'.format(MODEL_NAME)
    _TARBALL_PATH = os.path.join(MODEL_DIR, _TARBALL_NAME)
...
if __name__ == '__main__':
    ...
    try:
        ...
        print('running model...')
        resized_im, seg_map = MODEL.run(orignal_im, upscale=True)

        print("Orig: {}, Resized: {}, Color map: {}".format(orignal_im.size, resized_im.size, seg_map.shape))
        ...
        vis_segmentation(orignal_im, seg_map)
    except IOError:
